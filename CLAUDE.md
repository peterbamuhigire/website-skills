...
**Operations (required for retainers):** `project-log-template.md` → copied into the client project at `project-log/README.md` at kickoff. The log is mandatory for any project entering a retainer or receiving monthly reports.
...
├── project-log/           ← Append-only record of all changes, decisions, incidents, and metric snapshots
│   ├── README.md          ← Copied from templates/project-log-template.md at kickoff
│   ├── CHANGELOG.md       ← Daily entries, newest first, ISO dates, mandatory
│   ├── decisions/         ← ADR-format decision records (optional, as needed)
│   ├── incidents/         ← Incident records (optional, as needed)
│   └── monthly/           ← Monthly roll-ups — source for monthly-report skill
│       └── YYYY-MM.md
